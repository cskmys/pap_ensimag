...
.Parallel Merge Sort
image::./pmsort.png[Parallel Merge Sort]
...
.Parallel Bubble Sort
image::pbub.png[Parallel Bubble Sort]
...
.Parallel Sort Regular Sampling
image::psrs.png[Parallel Sort Regular Sampling]
...
* To evaluate the performance of in machine learning, loss functions is used. It is nothing but a multi-dimensional root mean square operation.
* Back-propagation algorithm effectively is recursive dot product of derivatives across multiple layers in neural nets.

Though, the operations are simple, the scale at which they are performed are unprecedented.
For example: a simple 'hello world' introduction into deep learning involves a model that requires several 256*256 matrix multiplications in one iteration of training.
...
Parallel algorithms provide the foundation on which makes all these are practically possible/feasible.
...
image::ringdatadist.png[Data allocation]
...
image::distlocsubmatmul.png[Local sub-matrix multiplication]
...
+
.Virtual ring to pass rows footnote:[source: Arnaud Legrand, LIG, Grenoble]
image::distmatmulring.png[Virtual ring to pass rows]
...
+
is the running time of matrix vector multiplication using the same algorithm.
Hence, by reduction in factor of we have saved network latencies!
...
image::sermat.png[Serial Matrix Multiplication]
...
image::openmpmatmul.png[Parallel Matrix Multiplication OpenMP]

==== Complexity analysis
The serial matrix multiplication as we can see contains 3 nested for loops, each iterating for say, latexmath:[n] dimension.
